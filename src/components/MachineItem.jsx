...
const MachineItem = ({ machine }) => {
  ...
  const machineName = language === 'ar' ? machine.machineNameAr : machine.machineName;
  const machineUse = language === 'ar' ? machine.machineUseAr : machine.machineUse;
  ...
  const dimensions = language === 'ar' ? machine.dimensionsAr : machine.dimensions;
  const heatingSystem = language === 'ar' ? machine.HeatingSystemAr : machine.HeatingSystem;
  const vibratorMotor = language === 'ar' ? machine.VibratorMotorAr : machine.VibratorMotor;
  const fanMotor = language === 'ar' ? machine.FanMotorAr : machine.FanMotor;
  const roasteryMotor = language === 'ar' ? machine.RoasteryMotorAr : machine.RoasteryMotor;

  // Helper function to render a table row only if the value is present and not "HP"
  const renderRow = (label, value) => {
    if (!value || value.trim() === '' || value.trim().toLowerCase() === 'hp') return null;
    return (
      <tr className={label === 'Voltage' ? 'bg-[#cfac6e]' : 'bg-[#353941]'}>
        <td className="py-2 px-4 font-semibold border border-gray-300">{label}</td>
          ...
          <table className="min-w-full bg-gray-100 border text-white border-gray-300">
            <thead>
              <tr className="bg-black text-white">
                <th className="py-2 px-4 text-center" colSpan="2">Machine Details</th>
              </tr>
            </thead>
            <tbody>
